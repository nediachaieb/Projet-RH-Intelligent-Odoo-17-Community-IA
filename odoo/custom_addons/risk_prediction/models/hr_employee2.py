...
class HrEmployee(models.Model):
    ...
    def predict_risk_for_employees(self):
        ...
        for rec in self:
            # if not rec.performance_rating:
            #     _logger.warning("L'évaluation de performance est manquante pour %s", rec.name)
            #     rec.predicted_risk = 'undefined'
            #     continue
            payload = {
                "age": rec.age or 0,
                "years_at_company": rec.years_at_company,
                "job_role": rec.department_id.name,
                "monthly_income": int(rec.monthly_income),
                "number_of_promotions": rec.number_of_promotions,
                "distance_from_home": int(rec.km_home_work),
                "number_of_dependents": rec.children,
                ...
                "leadership_opportunities": self._label(rec.leadership_opportunities ),
                "innovation_opportunities": self._label(rec.innovation_opportunities),
            ...
            try:
                ...
                if resp.status_code == 200:
                    ...
                    # Erreur HTTP (données invalides, service down…) –-----------
                    _logger.error(
                        "API Error for %s : status=%s – body=%s",
                        rec.name, resp.status_code, resp.text[:200]  # on log 200 caract. max.
                    )
                    risk = "undefined"  # on marque le risque comme indéfini

            except (requests.exceptions.RequestException, ValueError) as e:
                # Exception Python : time-out, JSON mal formé, etc. –-----------
                _logger.error("API Exception for %s : %s", rec.name, e)
                risk = "undefined"
